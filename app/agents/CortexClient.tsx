    ...
    return (
        <div className="min-h-screen pt-20 md:pt-24 pb-8 md:pb-12 px-2 sm:px-4 md:px-6 flex flex-col bg-black text-white">
            ...
            <div className="flex flex-col gap-4 mb-6 md:mb-8">
                <div>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tighter flex flex-wrap items-center gap-2 uppercase">
                        {t('neural')} <span className="text-transparent bg-clip-text bg-linear-to-r from-cyan-400 to-purple-600">{t('swarm_intel')}</span>
                    </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                        <Power size={16} />
                        {isArmed ? t('deactivate_swarm') : t('activate_swarm')}
                    </button>

                    <Link href="/agents/create" className="w-full sm:w-auto">
                        <button
                            className="w-full flex items-center justify-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full font-bold text-sm transition-all bg-cyan-500 text-black border border-cyan-500 hover:shadow-[0_0_20px_rgba(6,182,212,0.5)] md:hover:scale-105 uppercase"
                        >
                            {t('deploy_agent')}
                        </button>
                    </Link>
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
            ...
            <div className="glass-panel p-4 sm:p-6 flex flex-col justify-center relative overflow-hidden group">
                ...
                <h3 className="text-gray-500 font-mono text-[10px] sm:text-xs tracking-widest mb-1 flex items-center gap-2 uppercase">
                ...
                <div className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white font-mono mt-1 sm:mt-2">
                    {balance !== null ? (
                        ...
                        <span className="text-lg sm:text-xl md:text-2xl text-gray-600 animate-pulse uppercase">{t('connecting')}</span>
                    )}
                </div>
                <div className="font-mono text-[9px] sm:text-[10px] text-gray-600 mt-1 sm:mt-2 truncate uppercase">
                    ADDR: {stats ? stats.user.toString() : t('scanning_addr')}
                </div>
            </div>
            ...
            <div className="glass-panel p-4 sm:p-6 flex flex-col justify-center relative overflow-hidden group">
                ...
                <h3 className="text-gray-500 font-mono text-[10px] sm:text-xs tracking-widest mb-1 uppercase">
                    {t('vault_performance')}
                </h3>
                <div className="flex items-end gap-2 sm:gap-4 mt-1 sm:mt-2">
                    ...
                    <div className="text-xs sm:text-sm font-mono text-gray-400 mb-1 sm:mb-2 uppercase">
                        {t('accuracy')}
                    </div>
                </div>
                <div className="font-mono text-[9px] sm:text-[10px] text-gray-600 mt-1 sm:mt-2 uppercase">
                    {t('based_on')} {stats ? stats.predictionsCount.toString() : "0"} {t('verified_predictions')}
                </div>
            </div>
        </div>
    );
