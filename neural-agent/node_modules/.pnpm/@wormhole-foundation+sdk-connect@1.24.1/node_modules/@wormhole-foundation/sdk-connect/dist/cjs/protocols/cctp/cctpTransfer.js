...
class CircleTransfer {
    ...
    static async fromWormholeMessageId(wh, from, timeout) {
        ...
        const rcvChain = sdk_base_2.circle.toCircleChain(wh.network, vaa.payload.targetDomain);
    ...
    static async fromCircleMessage(wh, message) {
        const [msg, hash] = sdk_definitions_1.CircleBridge.deserialize(sdk_base_2.encoding.hex.decode(message));
        const { payload: burnMessage } = msg;
        const xferSender = burnMessage.messageSender;
        const xferReceiver = burnMessage.mintRecipient;
        const sendChain = sdk_base_2.circle.toCircleChain(wh.network, msg.sourceDomain);
        const rcvChain = sdk_base_2.circle.toCircleChain(wh.network, msg.destinationDomain);
        const details = {
...
(function (CircleTransfer) {
        ...
        if ((0, types_js_1.isAttested)(receipt) || (0, types_js_1.isSourceFinalized)(receipt)) {
            ...
            if ((0, sdk_definitions_1.isWormholeMessageId)(receipt.attestation.id)) {
                ...
                if (txStatus && txStatus.globalTx?.destinationTx?.txHash) {
                    ...
                    receipt = {
                        ...receipt,
                        destinationTxs: [{ chain: (0, sdk_base_2.toChain)(chainId), txid: txHash }],
    ...
    async function destinationOverrides(srcChain, dstChain, transfer) {
        ...
        if (transfer.to.chain === "Solana" && !_transfer.automatic) {
            const usdcAddress = wormhole_js_1.Wormhole.parseAddress("Solana", sdk_base_2.circle.usdcContract.get(dstChain.network, dstChain.chain));
    ...
    async function quoteTransfer(srcChain, dstChain, transfer) {
        if (!sdk_base_2.circle.isCircleChain(dstChain.network, dstChain.chain))
            throw new Error(`Invalid destination chain ${dstChain.chain} for Circle transfer`);
        const dstUsdcAddress = sdk_base_2.circle.usdcContract.get(dstChain.network, dstChain.chain);
        if (!dstUsdcAddress)
            throw "Invalid transfer, no USDC contract on destination";
        if (!sdk_base_2.circle.isCircleChain(srcChain.network, srcChain.chain))
            throw new Error(`Invalid source chain ${srcChain.chain} for Circle transfer`);
        const srcUsdcAddress = sdk_base_2.circle.usdcContract.get(srcChain.network, srcChain.chain);
        if (!srcUsdcAddress)
        ...
        const eta = (srcChain.chain === "Polygon" ? 2_000 * 200 : sdk_base_2.finality.estimateFinalityTime(srcChain.chain)) +
            sdk_base_2.guardians.guardianAttestationEta;
        const expires = transfer.automatic
            ? sdk_base_1.time.expiration(0, 5, 0) // 5 minutes for automatic transfers
            : sdk_base_1.time.expiration(24, 0, 0); // 24 hours for manual
        if (!transfer.automatic) {
            return {
