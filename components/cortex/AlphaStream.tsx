...
export default function AlphaStream({ armed }: { armed: boolean }) {
    ...
    return (
        <div className="glass-panel p-5 h-full flex flex-col overflow-hidden font-mono text-xs">
            <h3 className="text-gray-400 font-mono text-xs tracking-widest mb-4 border-b border-white/5 pb-2 flex justify-between items-center">
                <span>ALPHA STREAM v2.0</span>
                {armed && <span className="text-[10px] text-green-500 animate-pulse">● LIVE</span>}
            </h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                <AnimatePresence initial={false}>
                    {thoughts.map((log: AgentThought) => (
                        ...
                        >
                            ...
                            <span className={`w-14 text-center text-[10px] flex-shrink-0 self-start mt-0.5 ${getLogStyle(log.thought_type)}`}>
                                {log.thought_type.substring(0, 4)}
                            </span>
                            <span className="text-gray-300 flex-1 break-words leading-tight">
                ...
                {!armed && <div className="text-center text-gray-600 mt-10">SYSTEM INITIALIZING...</div>}
                {armed && thoughts.length === 0 && <div className="text-center text-gray-600 mt-10">WAITING FOR NEURAL FEED...</div>}
            </div>
        </div>
    );
}
