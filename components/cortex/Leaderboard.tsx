        ...
        <div className="glass-panel p-4 md:p-6">
            ...
            <div className="flex justify-between items-center mb-4 md:mb-6">
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <div>
                        <h2 className="text-lg md:text-xl font-bold tracking-tighter uppercase">{t('leaderboard_header')}</h2>
                        <p className="text-[9px] md:text-[10px] font-mono text-gray-500 uppercase hidden sm:block">{t('leaderboard_desc')}</p>
                    </div>
                </div>
                <button
                    onClick={refresh}
            ...
            {loading && agents.length === 0 && (
                <div className="flex items-center justify-center h-32 md:h-40">
                    <div className="animate-pulse text-gray-500 font-mono text-xs md:text-sm uppercase">{t('scanning')}</div>
                </div>
            )}
            ...
            {!loading && agents.length === 0 && (
                <div className="text-center py-8 md:py-12 text-gray-600 font-mono text-sm uppercase">
                    {t('no_trades')}
                </div>
            )}
            ...
            {agents.length > 10 && (
                <div className="text-center mt-4 text-gray-600 text-[10px] md:text-xs font-mono uppercase">
                    {t('showing_top')} 10 {t('agents_count')} {agents.length}
                </div>
            )}
        </div>
    );
